...
for i, domsize in enumerate(domsizes):
    ...
    wlowflat = Wlow_block.flatten()
    wmidflat = Wmid_block.flatten()
    whighflat = Whigh_block.flatten()
    ...
    nwlowhist, wlowedges = np.histogram(wlowflat, bins=wbins)
    wlowhist, wlowedges = np.histogram(wlowflat, bins=wbins, weights=np.zeros_like(wlowflat) + 1. / (nxprime*nxprime))
    
    nwmidhist, wmidedges = np.histogram(wmidflat, bins=wbins)
    wmidhist, wmidedges = np.histogram(wmidflat, bins=wbins, weights=np.zeros_like(wmidflat) + 1. / (nxprime*nxprime))
    
    nwhighhist, whighedges = np.histogram(whighflat, bins=wbins)
    whighhist, whighedges = np.histogram(whighflat, bins=wbins, weights=np.zeros_like(whighflat) + 1. / (nxprime*nxprime))
    
    
    print 'binning cloud fraction'
    lowcldhist, wedges = np.histogram(wlowflat, bins=wbins, weights=lowcloudfracflat)
    midcldhist, wedges = np.histogram(wmidflat, bins=wbins, weights=midcloudfracflat)
    highcldhist, wedges = np.histogram(whighflat, bins=wbins, weights=highcloudfracflat)
    
    lowcldhist = lowcldhist/nwlowhist
    midcldhist = midcldhist/nwmidhist
    highcldhist = highcldhist/nwhighhist
    
    wlowcenters = (wlowedges[:-1] + wlowedges[1:])/2.
    wmidcenters = (wmidedges[:-1] + wmidedges[1:])/2.
    whighcenters = (whighedges[:-1] + whighedges[1:])/2.
    
    lowindices = np.isfinite(lowcldhist)
    midindices = np.isfinite(midcldhist)
    highindices = np.isfinite(highcldhist)
    ...
    fig=plt.figure(2)
    plt.suptitle('W frequency distributions (left) and cloud fraction (right), low = p > {:3.0f} hPa, mid={:3.0f} hPa > p > {:3.0f} hPa, high={:3.0f} hPa > p > {:3.0f} hPa'.format(p[plow_ti], p[plow_ti], p[phigh_bi], p[phigh_bi], p[phigh_ti]))
    ax1 = fig.add_subplot(3,2,1)
    ax1.plot(wlowcenters[lowindices], wlowhist[lowindices], 'x-', color=colors[i], linewidth=3, label='{:d} km, day {:3.0f} to day {:3.0f} average'.format(domsize, times2D[t-aveperiod2D], times2D[t]))
    ax1.set_ylabel('low-level W frequency')
    plt.axvline(0, color='k', alpha=0.5)
    ax2 = fig.add_subplot(3,2,3)
    ax2.plot(wmidcenters[midindices], wmidhist[midindices], 'o-', color=colors[i], linewidth=3, label='{:d} km, day {:3.0f} to day {:3.0f} average'.format(domsize, times2D[t-aveperiod2D], times2D[t]))
    ax2.set_ylabel('mid-level W frequency')
    ax3 = fig.add_subplot(3,2,5)
    ax3.plot(whighcenters[highindices], whighhist[highindices], '^-', color=colors[i], linewidth=3, label='{:d} km, day {:3.0f} to day {:3.0f} average'.format(domsize, times2D[t-aveperiod2D], times2D[t]))
    ax3.set_ylabel('high-level W frequency')
    plt.axvline(0, color='k', alpha=0.5)
    ax4 = fig.add_subplot(3,2,2)
    ax4.plot(wlowcenters[lowindices], lowcldhist[lowindices], 'x-', color=colors[i], label='{:d} km, day {:3.0f} to day {:3.0f} average'.format(domsize, times2D[t-aveperiod2D], times2D[t]))
    ax4.set_ylabel('low cloud fraction (%)')
    plt.axvline(0, color='k', alpha=0.5)
    ax5 = fig.add_subplot(3,2,4)
    ax5.plot(wmidcenters[midindices], midcldhist[midindices], 'o-', color=colors[i], label='{:d} km, day {:3.0f} to day {:3.0f} average'.format(domsize, times2D[t-aveperiod2D], times2D[t]))
    ax5.set_ylabel('mid cloud fraction (%)')
    plt.axvline(0, color='k', alpha=0.5)
    ax6 = fig.add_subplot(3,2,6)
    ax6.plot(whighcenters[highindices], highcldhist[highindices], '^-', color=colors[i], label='{:d} km, day {:3.0f} to day {:3.0f} average'.format(domsize, times2D[t-aveperiod2D], times2D[t]))
    ax6.set_ylabel('high cloud fraction (%)')
    plt.axvline(0, color='k', alpha=0.5)
    ax4.set_ylim(0,100)
    ax5.set_ylim(0,100)
    ax6.set_ylim(0,100)
    wlowlim = 0
    wuplim = 1
    ax1.set_xlim(wlowlim, wuplim)
    ax2.set_xlim(wlowlim, wuplim)
    ax3.set_xlim(wlowlim, wuplim)
    ax4.set_xlim(wlowlim, wuplim)
    ax5.set_xlim(wlowlim, wuplim)
    ax6.set_xlim(wlowlim, wuplim)
    #ax6.set_xlabel('W500 (m/s)')
    

ax1.legend(loc='best')
plt.savefig(fout + 'Wvscloudfrac.pdf')
plt.close()
