...
for i in times:
    ...
    #field_tave = blockave2D(field_tave, db)
    varname_u = 'U200'
    U = varis2D['U200'][t2-nave2D:t2,:,:]
    U_tave = np.mean(U, axis=0)
    V = varis2D['V200'][t2-nave2D:t2,:,:]
    ...
    #plt.legend(loc='best')
    q = plt.quiver(xx[::12, ::12]/1e3, yy[::12, ::12]/1e3, U_tave[::12,::12], V_tave[::12,::12], scale=300, alpha=0.8, zorder=1)
    #q = plt.quiver(xx[::12, ::12]/1e3, yy[::12, ::12]/1e3, U_tave[::12,::12], V_tave[::12,::12], scale=200, alpha=0.8, zorder=1)
    if varname == 'BLQVADV':
        xxplot = xx[:-1, :-1]
        yyplot = yy[:-1, :-1] 
    else:
    ...
    cb = plt.colorbar()
    cb.set_label('PW ({0})'.format(units))
    p = plt.quiverkey(q, np.min(x)/1e3+30, np.max(y)/1e3+30, 10, "10 m/s",coordinates='data',color='k', alpha=0.8)
    #p = plt.quiverkey(q, np.min(x)/1e3+30, np.max(y)/1e3+30, 5, "5 m/s",coordinates='data',color='k', alpha=0.8)
    plt.xlabel('x (km)')
    plt.ylabel('y (km)')
    if varname == 'SFCSPEED':
    ...
    if varname == 'BLQVADV':
        ...
        titlename = varname_u
    if db == 1:
        plt.title('{:s} (m/s) at day = {:3.2f}'.format(titlename, times2D[t2]))
        ...
        plt.title('{:s} (m/s) at day = {:3.2f}, block-averaging over ({:2.0f} km)$^2$'.format(titlename, times2D[t2], db*(np.diff(x)[0])/1e3))
    ...
    #plt.legend(loc='best')
    plt.savefig(fout + 'U200{:s}map_day{:2.2f}db{:2.0f}.jpg'.format(varname, times2D[t2], db))
