...
for i, nc_f3D in enumerate(nc_3Ds):
    ...
    for k, t in enumerate(t3D):
        ...
        E_blockave = blockave2D(E[k2,:,:], db)
        alpha = E_blockave/P_blockave
        ...
        alpha_conv = alpha[convBL_points]
        idnx = np.isfinite(alpha_conv)
        alpha_conv = np.mean(alpha_conv[idnx])
    ...
    #alphaconv_smooth = moving_average(
    ...
    plt.figure(6)
    #plt.plot(t2D, Pbar, '--', color=pcolors[i], alpha=0.7, label='{:d} km, domain mean'.format(domsizes[i]))
    plt.plot(times, alphaconv, color=pcolors[i], alpha=0.7, label = '{:d} km, convective'.format(domsizes[i]))
    plt.plot(times[w/2:-w/2+1], Pconv_smooth, color=pcolors[i], linewidth=1.5)
    #plt.plot(times[w/2:-w/2+1], convfracs_smooth, color=pcolors[i], linewidth=1.5)
    #plt.plot(times, 1-convfracs, 'k--')
    plt.xlabel('time (days)')
    ...
    plt.gca().set_ylim(bottom=0)
    plt.title(r'$\alpha$')
    plt.savefig(fout + 'alpha250days_db{:d}.pdf'.format(db))
...
plt.figure(6)
plt.legend(loc='best')
plt.savefig(fout + 'alphadays_db{:d}.pdf'.format(db))
plt.close()
