...
fpath = '/glade/scratch/patrizio/OUT_3D_nc/'
...
nc_in4 = glob.glob(fpath + '*2048x2048*3000m*day282to288*302K.nc')[0]
...
ts2 = np.array([-1, -1, -1, -1])*ntave2D
ts3 = np.array([-1, -1, -1, -1])*ntave3D
...
for k, nc_in in enumerate(ncs):
   ...
   #     sigma[i] = len(Wz[Wz >= Wcrit])/(1.*totpoints)
    ...
    for i in np.arange(0, z.size-1):
        ...
        verthflux = (rhoz_tave[i+1]*Wh_up - rhoz_tave[i]*Wh_down)/delz
        #total vertical mass flux convergence in convective region
        #divs[i] = -np.sum(vertmassflux)*delx*dely*(db**2)
        divs[i] = -np.nanmean(vertmassflux)*delz
        divh[i] = -np.nanmean(verthflux)*delz
    ...
    print 'vertical integral of MSE divergence (W/m^2)', np.nansum(divh)
