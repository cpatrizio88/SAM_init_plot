...
fpath2D = '/glade/scratch/patrizio/OUT_2D_nc/OLD/'
fpath3D = '/glade/scratch/patrizio/OUT_3D_nc/OLD/'
...
for i, domsize in enumerate(domsizes): 
    ...
    QN = QN - np.mean(np.mean(QN,axis=1,keepdims=True),axis=1,keepdims=True)
    ...
    hprimesq_tave = np.mean(hprimesq,axis=0)
    hprimesq_tavebar = np.mean(np.mean(hprimesq_tave,axis=0),axis=0)
    
    norm_factor = (3600*24)/hprimesq_tavebar
     #hprimesq_tavebar = 1    
    print 'hprimesq_tavebar', hprimesq_tave
    M = -(Enet[1:-1,:,:] - dhhatdt)
    M = np.multiply(hprime, M)
    M_tave = np.mean(M,axis=0)*norm_factor
    ...
    if hprimeflag:
       QN = QN*norm_factor
       QN = np.multiply(hprime,QN[1:-1,:])
   
    hprime_tave = np.mean(hprime,axis=0)
...
#      
#     
#     hprimerbins, hprimemeans = radprof(hprime_tave, xx, yy, mcenter, bins)
#     hprimerbin_centers = (hprimerbins[1:] + hprimerbins[:-1])/2.
    ...
    for vi, varname in enumerate(varnames):
        ...
        #    W
            ...
            if hprimeflag:
               field = field.reshape(nt2D/6,6,nx,ny)
               field = np.mean(field,axis=1)
               field = field - np.mean(np.mean(field,axis=1,keepdims=True),axis=1,keepdims=True)
               field = np.multiply(hprime, field[1:-1,...])
        ...
        if varname == 'PW':
            ...
            titlename = r'$\langle h^{\prime} \rangle SHF^{\prime}$'
            #ax.set_ylim([-1,1])
        elif varname == 'LHF':
            titlename = r'$\langle h^{\prime} \rangle LHF^{\prime}$'
            #ax.set_ylim([-1,1])
        else: 
            titlename = varname
        ...
        plt.figure()
        plt.contourf(xx/1e3, yy/1e3, field_tave, levels=Mnormlevels, cmap=plt.cm.RdBu_r, extend='both')
        plt.title(titlename + ', day{:3.0f} to {:3.0f} average'.format(t2D[0], t2D[-1]))
        plt.colorbar(label=r'day$^{-1}$')
        plt.xlabel('x (km)')
        plt.ylabel('y (km)')
        plt.savefig(fout + '{:d}km_normhprime{:s}map_day{:3.0f}to{:3.0f}.pdf'.format(domsize, varname, t2D[0], t2D[-1]))
        plt.close()
        
        plt.figure()
        plt.contourf(xx/1e3, yy/1e3, M_tave, levels=Mnormlevels, cmap=plt.cm.RdBu_r, extend='both')
        plt.title('$M$, day{:3.0f} to {:3.0f} average'.format(t2D[0], t2D[-1]))
        plt.colorbar(label=r'day$^{-1}$')
        plt.xlabel('x (km)')
        plt.ylabel('y (km)')
        plt.savefig(fout + '{:d}km_normhprimeMmap_day{:3.0f}to{:3.0f}.pdf'.format(domsize, varname, t2D[0], t2D[-1]))
        plt.close()


        plt.figure()
        plt.contourf(xx/1e3, yy/1e3, QN_tave, levels=Mnormlevels, cmap=plt.cm.RdBu_r, extend='both')
        plt.title(r'$\langle h^{\prime} \rangle Q^{\prime}_{net}$, day{:3.0f} to {:3.0f} average'.format(t2D[0], t2D[-1]))
        plt.colorbar(label=r'day$^{-1}$')
        plt.xlabel('x (km)')
        plt.ylabel('y (km)')
        plt.savefig(fout + '{:d}km_normhprimeQnetmap_day{:3.0f}to{:3.0f}.pdf'.format(domsize, varname, t2D[0], t2D[-1]))
        plt.close()
        ...
        # ax.set_ylim([-1.0,1.0])
        # 
        # plt.savefig(fout + 'hprime{:s}radialprof_day250_{:d}day.pdf'.format(varname, nave))
        #plt.close()
...
#     plt.ylim([-1.0,1.0])
#     plt.xlim([0, (1/np.sqrt(2))])
#     plt.savefig(fout + 'hprimeQNetradialprof_day250_{:d}day.pdf'.format(nave))
# 
#     
#     #fig = plt.figure()
#     #ax = fig.gca()
...
# 
#     plt.figure(30)
#     plt.plot(dhhatdtbin_centers/(domsize*1e3), Enetmeans + Mmeans, '-', color=colors[i],  label='{:d} km'.format(domsize))
#     plt.xlabel(r'$\hat{r}$', fontsize=34)
#     plt.ylabel(r'$\partial \langle  h^{\prime} \rangle^2 / \partial t$ (day$^{-1}$)')
#     plt.title(r'$\partial \langle  h^{\prime} \rangle^2 / \partial t$')
#     plt.ylim(-1.0, 1.0)
#     plt.xlim([0, (1/np.sqrt(2))])
#     plt.axhline(0, color='black', linewidth=1)
#     plt.savefig(fout + 'EnetMradialprof_day250_{:d}day.pdf'.format(nave))
# 
#     plt.figure(31)
#     plt.plot(hprimerbin_centers/(domsize*1e3), hprimemeans, '-', color=colors[i],  label='{:d} km, day {:3.0f} to {:3.0f} mean'.format(domsize, t2D[0], t2D[-1]))
#     plt.xlabel(r'$\hat{r}$', fontsize=34)
#     plt.ylabel(r'$\langle h^{\prime} \rangle$ (J m$^{-2}$)')
#     plt.title(r'$\langle h^{\prime} \rangle$')
#     #plt.ylim([-0.3, 0.8])
#     plt.xlim([0, (1/np.sqrt(2))])
#     plt.axhline(0, color='black', linewidth=1)
#     plt.savefig(fout + 'hprimeradialprof_day250_{:d}day.pdf'.format(nave))  
# 
# for vi, varname in enumerate(varnames):
#     plt.figure(vi)
#     plt.legend(loc='best')
...
#     plt.close()
#     
# varnames = ['QNTOA', 'QNet', 'SWN', 'LWN', 'SWNclear', 'LWNclear', 'LWNcloud', 'SWNcloud', 'M', 'QnetSHFGMS', 'QnetLHFGMS', 'QnetLHF', 'LHFGMS', 'dhhatdt', 'EnetM', 'hprime']
#     
