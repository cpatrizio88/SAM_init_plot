...
np.save(fout + '{:d}km_buoyflux_day{:3.0f}to{:3.0f}QP'.format(domsize, t3D[0], t3D[-1]), buoyfluxnew)

buoyfluxbar_zt = np.mean(np.mean(buoyfluxnew, axis=2), axis=2)
...
plt.ylabel(r'buoyancy flux (W/m$^2$)')
plt.xlabel('t (days)')
plt.title(r'evolution of daily averaged domain-mean buoyancy flux, domain size ({:d} km)$^2$'.format(domsize)) 
