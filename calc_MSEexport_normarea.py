...
for j, domsize in enumerate(domsizes):
    ...
    for k, nc_3D in enumerate(nc_3Ddom):
        ...
        P = P[ntrunc:,:,:]
        P = P.reshape(nt3D, 6, nx, ny)
        P = np.mean(P, axis=1)
        
        Enet_s = Enet_s.reshape(nt3D, 6, nx, ny)
        Enet_s = np.mean(Enet_s, axis=1)
        ...
        plt.axhline(0, color='b')
        plt.xlabel('time (days)')
        plt.ylabel(r'MSE export from convective region (W m$^{-2}$)')
        plt.savefig(fout + 'MSEexportnorm.pdf')
